...
if uploaded_file:
    ...
    # COMPARACIÓN DE TIEMPOS SEGÚN K
    ...
    tiempos_df = pd.DataFrame(tiempos_k)

    # IDENTIFICAR MEJOR Y PEOR

    mejor_tiempo = tiempos_df["Tiempo"].min()
    peor_tiempo = tiempos_df["Tiempo"].max()

    colores_tiempo = []

    for valor in tiempos_df["Tiempo"]:

        if valor == mejor_tiempo:
            colores_tiempo.append("green")

        elif valor == peor_tiempo:
            colores_tiempo.append("red")

        else:
            colores_tiempo.append("#FFA500")

    tiempos_df["Color"] = colores_tiempo

    st.header("⏱ Comparación de Tiempo por Número de Clusters")
    ...
    fig_tiempos = px.bar(
        ...
        title="Tiempo de ejecución para cada K"
    )
