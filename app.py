...
# ============================================================
# FUNCIONES Y VISOR DE PDF OPTIMIZADO
# ============================================================
def article_path():
    candidates = [
...
def pdf_viewer(bytes_data, page=1, height=750):
    """
    Renderizador PDF compatible y seguro mediante PDF.js vía CDN.
    Soluciona el bloqueo de iframe data:application/pdf en navegadores modernos.
    """
    ...
    <head>
      ...
      <style>
        ...
        body {{ margin: 0; padding: 0; background: #F8FAFC; text-align: center; }}
      </style>
    </head>
    <body>
      <canvas id="pdf-render"></canvas>
      <script>
        const pdfData = atob("{encoded}");
...
with left:
    ...
    if pdf_bytes is not None:
        ...
        st.download_button(
            ...
            data=pdf_bytes,
            file_name=local_pdf.name if local_pdf else "articulo.pdf",
            mime="application/pdf"
        )
    else:
        st.error(
            "⚠️ No se encontró el archivo PDF en la carpeta de la app. Asegúrate de renombrar tu PDF como 'intento suicida.pdf' e incluirlo junto a este código."
        )

with right:
    # 01 INTRODUCCIÓN
    if section == "01 · Introducción":
        section_header("1", "Entrar al estudio", "Antes de hablar de modelos, conozcamos qué estudiaron los autores y dónde ocurrió.")
        a, b = st.columns(2)
        with a:
            card("¿Qué investigaron?", "El comportamiento epidemiológico del intento de suicidio y las diferencias entre género y variables sociodemográficas, psicosociales y específicas durante 2012–2017.", "🎯")
        with b:
            card("¿Dónde?", "Municipio de <b>Sogamoso, Boyacá, Colombia</b>. El estudio utilizó casos reportados al SIVIGILA.", "📍")
    ...
    # 02 CONTEXTO
        ...
        section_header("2", "Del problema de salud pública a la pregunta estadística", "La investigación nace de un contexto territorial concreto.")
        st.markdown(
            """
            <div class="card">
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown('<div class="big-question">🔎 ¿Qué queremos explicar estadísticamente?</div>', unsafe_allow_html=True)
    ...
    # 03 DATOS Y DISEÑO
        ...
        section_header("3", "¿Cómo se construyó la información?", "Aquí seguimos el recorrido de los datos antes de analizarlos.")
        cols = st.columns(5)
        steps = [
    ...
    # 04 PREVALENCIA
        ...
        section_header("4", "Prevalencia ajustada por edad: ¿dónde se concentra el fenómeno?", "Esta es una de las figuras centrales del artículo. No es una tasa anual simple: cruza año y grupo de edad.")
        mode = st.radio("Forma de observar la Figura 1", ["Curvas por año", "Mapa de calor", "Comparar un año"], horizontal=True)
        if mode == "Curvas por año":
            fig = px.line(
                prev_df, x="Grupo de edad", y="Prevalencia", color="Año", markers=True,
                category_orders={"Grupo de edad": age_groups},
                labels={"Grupo de edad": "Grupo de edad (años)", "Prevalencia": "Prevalencia ajustada", "Año": "Año"},
            )
            fig.update_layout(height=510, margin=dict(l=10, r=10, t=30, b=10), legend_title="Año", xaxis_tickangle=-45)
            st.plotly_chart(fig, use_container_width=True)
            st.markdown(
                """
                <div class="interpretation">
                <b>Lectura estadística:</b>
                el patrón más marcado aparece en edades tempranas. El grupo de <b>15–19 años</b> presenta valores elevados a lo largo del periodo;
                el artículo señala un pico de 22,3 por 100.000 habitantes al inicio y 21 por 100.000 al final del periodo.
                </div>
                """,
                unsafe_allow_html=True,
            )
        elif mode == "Mapa de calor":
            heat = prev_df.pivot(index="Grupo de edad", columns="Año", values="Prevalencia").reindex(age_groups)
            fig = px.imshow(heat, text_auto=".1f", aspect="auto", labels={"x": "Año", "y": "Grupo de edad", "color": "Prevalencia"})
            ...
            fig = px.bar(d, x="Grupo de edad", y="Prevalencia", text="Prevalencia", labels={"Grupo de edad": "Grupo de edad", "Prevalencia": "Prevalencia ajustada"})
            fig.update_traces(texttemplate="%{text:.1f}", textposition="outside")
            fig.update_layout(height=500, margin=dict(l=10, r=10, t=30, b=10), xaxis_tickangle=-45)
            st.plotly_chart(fig, use_container_width=True)
        
        st.markdown("### ¿Cómo interpretarla sin equivocarnos?")
        a, b = st.columns(2)
        with a:
            card("No mirar solamente el máximo", "La figura debe leerse como una estructura de edades a través del tiempo. El interés está en la concentración y en su comportamiento, no solo en encontrar el año con el valor más alto.", "👁️")
        with b:
            card("Conclusión de los autores", "Las prevalencias ajustadas por edad no muestran una reducción significativa desde el inicio hasta el final del periodo y existe concentración en edades tempranas.", "📌")
    ...
    # 05 TABLA 1
        ...
        section_header("5", "Tabla 1: primero conozcamos quiénes están en los datos", "La estadística descriptiva permite descubrir patrones antes de llegar al modelo.")
        ...
        chart_df = d.melt(id_vars=["Categoría"], value_vars=["Mujer %", "Hombre %"], var_name="Sexo", value_name="Porcentaje")
        chart_df["Sexo"] = chart_df["Sexo"].str.replace(" %", "", regex=False)
        fig = px.bar(chart_df, x="Categoría", y="Porcentaje", color="Sexo", barmode="group", text="Porcentaje", labels={"Porcentaje": "% dentro del sexo"})
        fig.update_traces(texttemplate="%{text:.1f}", textposition="outside")
        fig.update_layout(height=470, margin=dict(l=10, r=10, t=30, b=10), xaxis_tickangle=-35)
        st.plotly_chart(fig, use_container_width=True)
    ...
    # 06 MODELO LOGÍSTICO
        ...
        section_header("6", "¿Por qué aparece la regresión logística?", "Aquí comienza la parte matemática de la exposición.")
        st.markdown('<div class="big-question">La variable de interés es binaria: queremos modelar la pertenencia al grupo de género.</div>', unsafe_allow_html=True)
        st.latex(
            r"""
            Y_i =
            \begin{cases}
    ...
    # 07 TABLA 2
        ...
        section_header("7", "Modelo multivariado final (Tabla 2)", "Resultados estimados en el artículo para el modelo multivariado.")
        ...
        fig.update_layout(
            ...
            xaxis_title="OR (Escala sugerida)",
            height=480,
    ...
    # 08 INTERPRETACIÓN DEL OR
        ...
        section_header("8", "¿Cómo interpretar el Odds Ratio (OR)?", "Relación entre el coeficiente estimado B y la razón de momios e interpretación práctica.")
        ...
        with col_a:
            card("Consumo de Alcohol (Sí)", "<b>B = 1.28</b><br><b>OR = e^(1.28) ≈ 3.58</b><br><i>(IC 95%: 2.17 – 5.90, p < 0.001)</i><br><br>Los momios asociados a la categoría de interés son 3.58 veces mayores cuando se reporta consumo de alcohol.", "🍺")
        with col_b:
            card("Edad: Adolescencia", "<b>B = -1.04</b><br><b>OR = e^(-1.04) ≈ 0.35</b><br><i>(IC 95%: 0.17 – 0.74, p = 0.01)</i><br><br>El OR menor que 1 indica una reducción significativa en los momios respecto a la categoría de referencia.", "📉")
        source_box("Interpretación metodológica del Odds Ratio.")

    # 09 EVALUACIÓN DEL MODELO
    elif section == "09 · Evaluación del modelo":
        section_header("9", "Evaluación de la bondad de ajuste del modelo", "Pruebas estadísticas para verificar la validez del modelo multivariado.")
        c_a, c_b = st.columns(2)
        with c_a:
            card("Prueba de Hosmer-Lemeshow", "Evalúa la coincidencia entre las frecuencias observadas y esperadas por el modelo.<br><br>Un valor p > 0.05 indica que no hay diferencia significativa entre lo observado y lo predicho, sugiriendo un <b>buen ajuste del modelo</b>.", "🧪")
        with c_b:
            card("Estadístico de Wald", "Evalúa la significancia individual de cada coeficiente beta en el modelo.<br><br>Valores de p < 0.05 sugieren que la variable aporta significativamente a la predicción del modelo.", "📊")
        source_box("Diagnóstico y validación del modelo estadístico.")

    # 10 DISCUSIÓN
    elif section == "10 · Discusión":
        section_header("10", "Discusión e integración con la literatura", "Comparación de los hallazgos con otros estudios epidemiológicos.")
        st.markdown(
            """
            <div class="card">
    ...
    # 11 CONCLUSIONES
        ...
        section_header("11", "Conclusiones y recomendaciones de salud pública", "Implicaciones de la investigación para la toma de decisiones.")
        a, b = st.columns(2)
        with a:
            card("Conclusiones Estadísticas", "El modelo multivariado permitió identificar que factores como la edad, el consumo de alcohol y ciertos desencadenantes psicosociales presentan asociaciones estadísticamente significativas.", "📌")
        with b:
            card("Recomendaciones en Salud Pública", "Fortalecer las estrategias preventivas dirigidas a adolescentes y adultos jóvenes, priorizando la intervención temprana en el consumo de alcohol y la resolución de conflictos familiares.", "🏛️")
